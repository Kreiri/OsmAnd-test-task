...
class MapsListAdapter(
    ...
    fun setItems(indexPath: List<Int>, regions: List<Region>) {
        items = buildItems(indexPath, regions)
    ...
    private fun buildItems(indexPath: List<Int>, regions: List<Region>): List<MapListItem> {
        ...
        regions.forEachIndexed { regionIndex, region ->
            ...
            if (region.type == RegionType.CONTINENT) {
                ...
                list.add(MapListItem.RegionItem(region, regionPath))
            }
        }
        return list.toList()
    }
